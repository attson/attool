...
打 `v*` tag 自动建 GitHub Release 草稿；`workflow_dispatch` 手动跑则只上传 14 天有效的 artifact。无 macOS / Windows 代码签名（用户首次打开会触发 Gatekeeper / SmartScreen），但 Tauri updater 自己的签名是有的（独立机制，由 `TAURI_SIGNING_PRIVATE_KEY` 注入）。
