...
**前端** `src/components/http/`（含 HttpTool + Sidebar + TabBar + [Sse|Ws]Tool + [Sse|Ws]RequestEditor + StreamMessageList + EnvModal + curl.ts / variables.ts / httpApi.ts / streamApi.ts / streamMessageTone.ts / types.ts）+ `src/composables/useHttpStore.ts`（http + stream 都走这个单例）

**后端** `src-tauri/src/http/`（mod + models + send（含 multipart）+ cancel + storage（4 表 + `kind` 列迁移）+ commands）+ `src-tauri/src/http/stream/`（session + parser + buffer + sse + ws + commands）
