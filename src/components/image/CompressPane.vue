...
import { NAlert, NButton, NInput, NInputGroup, NSlider } from 'naive-ui';
import { invoke } from '@tauri-apps/api/core';
import { open } from '@tauri-apps/plugin-dialog';
import Panel from '../ui/Panel.vue';

interface CompressItemResult {
  inputPath: string;
  outputPath: string;
  originalSize: number;
...
const notice = ref('');
const succeeded = ref<CompressItemResult[]>([]);
const failed = ref<CompressFailure[]>([]);

const totalOriginal = computed(() =>
  succeeded.value.reduce((sum, item) => sum + item.originalSize, 0)
);
const totalCompressed = computed(() =>
  succeeded.value.reduce((sum, item) => sum + item.compressedSize, 0)
);
const totalReductionPct = computed(() => {
  if (totalOriginal.value === 0) return 0;
  return Math.round(((totalOriginal.value - totalCompressed.value) / totalOriginal.value) * 100);
});
...
async function startCompress() {
  ...
  try {
    const response = await invoke<CompressResponse>('compress_images', {
      request: {
  ...
  <div class="compress-pane">
    <div class="grid">
      ...
      <Panel title="参数">
        <div class="form">
          <label class="field">
            <span class="lbl">质量（1-100，越低越小）</span>
          ...
          <label class="field">
            ...
            <n-input-group>
              <n-input v-model:value="outputDir" placeholder="/Users/you/Pictures/compressed" />
    ...
    <Panel v-if="succeeded.length > 0 || failed.length > 0" title="结果">
      <template #right>
        <span v-if="succeeded.length > 0" class="mono">
          总量 {{ formatSize(totalOriginal) }} → {{ formatSize(totalCompressed) }}
          <span class="tag-reduction">-{{ totalReductionPct }}%</span>
        </span>
      </template>
